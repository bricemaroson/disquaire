...
#changer permet d'afficher la page qui permet l'affichage de l'item
...
class BookingInline(admin.TabularInline, AdminURLMixin):
    model = Booking #TabluarInline: permet d'afficher les informations sur plusieurs lignes
    extra = 0 #aucune ligne additionnelle
    readonly_fields = ["created_at", "album_link", "contacted"]
    fields = ["created_at", "album_link", "contacted"]
    verbose_name = "Réservation" #verbose: change le nom sur admin
...
class ContactAdmin(admin.ModelAdmin):
    inlines = [BookingInline,]     #inline: affiche les information 
